...
export function Header({ onMenu }: HeaderProps) {
  return (
    <header className="sticky top-0 z-30 border-b border-edge/60 bg-bg/70 backdrop-blur-md">
      <div className="flex h-16 items-center gap-3 px-4 md:px-6">
        <button
          className="grid h-10 w-10 place-items-center rounded-xl border border-edge/60 text-muted hover:text-ink lg:hidden"
          onClick={onMenu}
          aria-label="Меню"
        >
          <IconMenu />
        ...
        <Link to="/" className="group flex items-center gap-2.5">
          <span className="relative grid h-9 w-9 place-items-center">
            <span className="absolute inset-0 rounded-lg bg-gradient-to-br from-accent to-accent2 opacity-90" />
